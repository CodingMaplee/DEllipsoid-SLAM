...
public:
    ...
    Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,
          ...
          const std::vector<Detection::Ptr>& detections, set<int> &mvDynamicCatId, const uchar* personMask, const cv::Mat &imRgb, GeometricCamera* pCamera,
    ...
    vector<cv::KeyPoint> GetFeaturesInBox(Eigen::Vector4d box, std::vector<cv::KeyPoint> keyPoint_curr);
    vector<Eigen::Vector3d> GetMapPoints(uchar* personMask, Eigen::Matrix<double, 3, 4> Rt_double, cv::Mat imDepth);
    vector<size_t> GetFeaturesInMask(uchar* mask, std::vector<cv::KeyPoint> keyPoint_curr, Eigen::Vector4d bbox);
...
public:
    ...
    //std::vector<cv::KeyPoint> GetFeaturePoint2d(vector<int> point2d_index);
...
public:
    ...
    std::vector<cv::KeyPoint> GetKeyPoints_curr(cv::Mat im);
