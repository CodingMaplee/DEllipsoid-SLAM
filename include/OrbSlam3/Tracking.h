...
public:
    ...
    Sophus::SE3f GrabImageRGBD(const std::vector<Detection::Ptr>& detections, const uchar* personMask, const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename);
    ...
    bool GetStepByStep();
    bool TrackDynamicReferenceKeyFrame();
    // Load new settings
    ...
    void RemoveMapObject(ObjectTrack::Ptr track);
...
protected:
    ...
    bool Relocalization();
    bool RelocalizationFromObjects(bool use_points);
    void UpdateLocalMap();
    ...
    std::vector<MapPoint *>collectMapPoint3d;
    size_t current_frame_idx_ = 0;
    bool createdNewKeyFrame_ = false;
    bool collectMapPoint3d_ = true;
