...
LONG StreamingFunc(void* lParam);
...
public:
	CUDASceneRepChunkGrid(CUDASceneRepHashSDF* sceneRepHashSDF, const vec3f& voxelExtends, const vec3i& gridDimensions, const vec3i& minGridPos, unsigned int initialChunkListSize, bool streamingEnabled, unsigned int streamOutParts)	{
		...
		m_maxNumberOfSDFBlocksIntegrateFromGlobalHash = 20000;//100000;
	...
	std::cout<<"[MSG s_streamingEnabled] CUDASceneRepChunkGrid step 1" << std::endl;
	...
	void streamOutToCPUPass0GPU(const vec3f& posCamera, float radius, bool useParts, bool multiThreaded = true, bool needSave = true);
			...
			if (m_grid[i] != NULL)	{
				...
				for (unsigned int k = 0; k < descs.size(); k++) {
					...
					for (unsigned int l = 0; l < linearBlockSize; l++) {
						if (blocks[k].data[l].weight > 0 && fabsf(blocks[k].data[l].sdf) <= thresh) {
...
std::cout<<"[MSG s_streamingEnabled] CUDASceneRepChunkGrid  startMultiThreading step 1" << std::endl;
		initializeCriticalSection();
std::cout<<"[MSG s_streamingEnabled] CUDASceneRepChunkGrid  startMultiThreading step 2" << std::endl;
		s_terminateThread = false;

		pthread_create(&hStreamingThread,NULL,StreamingFunc,(void *)this);		 
	...
	void stopMultiThreading() {
		...
		if (!s_terminateThread) {
			...
			sem_post(&hEventOutProduce);
			sem_post(&hEventOutConsume);

			sem_post(&hEventInProduce);
			sem_post(&hEventInConsume);
			
			void* rval = nullptr;
			pthread_join(hStreamingThread, &rval);
	...
	void deleteCritialSection() {
		sem_destroy(&hMutexOut);
		sem_destroy(&hEventOutProduce);
		sem_destroy(&hEventOutConsume);

		sem_destroy(&hMutexIn);
		sem_destroy(&hEventInProduce);
		sem_destroy(&hEventInConsume);

	    sem_destroy(&hMutexSetTransform);
		sem_destroy(&hEventSetTransformProduce);
		sem_destroy(&hEventSetTransformConsume);
	}

	void initializeCriticalSection() {
	  sem_init(&hMutexOut,0,1);
	  sem_init(&hEventOutProduce,0,1);
	  sem_init(&hEventOutConsume,0,0);
	  
	  sem_init(&hMutexIn,0,1);
	  sem_init(&hEventInProduce,0,1);
	  sem_init(&hEventInConsume,0,0);
	  
	  sem_init(&hMutexSetTransform,0,1);
	  sem_init(&hEventSetTransformProduce,0,1);
	  sem_init(&hEventSetTransformConsume,0,0);
	}
	...
	// Multi-threading
	pthread_t hStreamingThread;
	DWORD dwStreamingThreadID;

	// Mutex
	sem_t hMutexOut;
	sem_t hEventOutProduce;
	sem_t hEventOutConsume;

	sem_t hMutexIn;
	sem_t hEventInProduce;
	sem_t hEventInConsume;

	sem_t hMutexSetTransform;
	sem_t hEventSetTransformProduce;
	sem_t hEventSetTransformConsume;
