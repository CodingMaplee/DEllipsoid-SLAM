...
public:
    ...
    MarchingCubesData* extractObjectSurfaceGPUNoChrunk ( const HashDataStruct& hashData, const HashParams& hashParams, const RayCastData& rayCastData, uchar objCat, const vec3f& minCorner = vec3f(0.0f, 0.0f, 0.0f), const vec3f& maxCorner = vec3f(0.0f, 0.0f, 0.0f), bool boxEnabled = false);
    void clearObjectSurfaceGPUNoChrunk ( const HashDataStruct& hashData, const HashParams& hashParams, const RayCastData& rayCastData, uchar objCat, const vec3f& minCorner = vec3f(0.0f, 0.0f, 0.0f), const vec3f& maxCorner = vec3f(0.0f, 0.0f, 0.0f), bool boxEnabled = false);

    //MarchingCubesData* extractDynamicSurfaceGPUNoChrunk(const HashDataStruct& hashData, const HashParams& hashParams, const RayCastData& rayCastData, const vec3f& minCorner, const vec3f& maxCorner, bool boxEnabled = true);
