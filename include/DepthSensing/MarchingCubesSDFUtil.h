...
//#endif
	void extractIsoSurfaceAtPosition(const float3& worldPos, const HashDataStruct& hashData, const RayCastData& rayCastData)
	{
        ...
        /*if(v.mask>0 && v.mask != MINF)
        {
            *//*const unsigned char colors[27][3] = {
            ...
            {0, 43, 96},   {158, 114, 140}, {79, 184, 17}, {158, 193, 255}, {149, 158, 123}, {255, 123, 175}, {158, 8, 0}}; //{0, 0, 0},{0, 0, 255},     {255, 0, 0},   {0, 255, 0},

            unsigned char r = colors[(27 % v.mask)][0];
            unsigned char g = colors[(27 % v.mask)][1];
            unsigned char b = colors[(27 % v.mask)][2];
            v.color = make_uchar4(r,g,b,255);*//*
        }
        else{
            unsigned char gray;
            gray = (v.color.x+v.color.y+v.color.z)/3/5;
            v.color = make_uchar4(v.color.x,v.color.y,v.color.z,v.255);
        }*/
		Vertex vertlist[12];
    ...
    __device__
    void extractDynamicSurfaceAtPosition(const float3& worldPos, const HashDataStruct& hashData, const RayCastData& rayCastData)
	{
		const HashParams& hashParams = c_hashParams;
		const MarchingCubesParams& params = *d_params;
