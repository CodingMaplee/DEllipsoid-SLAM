...
extern "C" void resetCUDA(HashDataStruct& hashData, const HashParams& hashParams, unsigned int blockOffset = 0);
...
//extern "C" void insertMaskMapCUDA(HashDataStruct& hashData, const HashParams& hashParams, DepthCameraData* depthCameraData, const DepthCameraParams& depthCameraParams);
...
extern "C" void gen_virtual_Image_CUDA(uchar3* img, HashDataStruct& hashData, const HashParams& hashParams, const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams);
extern "C" void calculateSDFMapFromVoxelCUDA(float* sdfMap, HashDataStruct& hashData, const HashParams& hashParams, const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams);
...
public:
    ...
    /*void insertMask(const mat4f& lastRigidTransform, DepthCameraData* depthCameraData, const DepthCameraParams& depthCameraParams, unsigned int* d_bitMask) {
        ...
        depthCameraParams;
        insertMaskMap(depthCameraData, depthCameraParams);
    ...
    void calculateSDFMap(float* sdfMap, const Eigen::Matrix4f& lastRigidTransform, const DepthCameraData& depthCameraData, DepthCameraParams& depthCameraParams, unsigned int* d_bitMask)
    {
        ...
        calculateSDFMapFromVoxel(sdfMap, depthCameraData, depthCameraParams);
    }
    void removeExistingDynamicPx(const Eigen::Matrix4f& lastRigidTransform, const DepthCameraData& depthCameraData, DepthCameraParams& depthCameraParams, unsigned int* d_bitMask)
    {
        bindDepthCameraTextures(depthCameraData, depthCameraParams);
        ...
        removeExistingDynamicPx2DepthMap(depthCameraData, depthCameraParams);

    }
	void gen_image(uchar3 * img, const Eigen::Matrix4f& lastRigidTransform, const DepthCameraData& depthCameraData, DepthCameraParams& depthCameraParams, uchar* mask_gpu,unsigned int* d_bitMask)
	{
		bindDepthCameraTextures(depthCameraData, depthCameraParams);
		...
		gen_voxel_image(img, depthCameraData, depthCameraParams, mask_gpu);

	}
    void gen_virtual_image(uchar3 * img, const Eigen::Matrix4f& lastRigidTransform, const DepthCameraData& depthCameraData, DepthCameraParams& depthCameraParams,unsigned int* d_bitMask)
    {
        bindDepthCameraTextures(depthCameraData, depthCameraParams);
        ...
        gen_virtual_voxel_image(img, depthCameraData, depthCameraParams);

    }
	void integrate(const Eigen::Matrix4f& lastRigidTransform, const DepthCameraData& depthCameraData, DepthCameraParams& depthCameraParams, unsigned int* d_bitMask) {//, uint* existingDynamicPx
		...
		//volumetrically integrate the depth data into the depth SDFBlocks
		integrateDepthMap(depthCameraData, depthCameraParams);//, existingDynamicPx

		//garbageCollect();

		m_numIntegratedFrames++;
	}
    /*void integrate(const mat4f& lastRigidTransform, const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams, unsigned int* d_bitMask, const float* consistency_optical_flow_px) {
        ...
        //volumetrically integrate the depth data into the depth SDFBlocks with optical flow consistency.
        integrateDepthMapAndConsistency(depthCameraData, depthCameraParams, consistency_optical_flow_px);

        //garbageCollect();

        m_numIntegratedFrames++;
    }*/
	void deIntegrate(const Eigen::Matrix4f& lastRigidTransform, const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams, unsigned int* d_bitMask) {//, uint* existingDynamicPx
		...
		//volumetrically integrate the depth data into the depth SDFBlocks
		deIntegrateDepthMap(depthCameraData, depthCameraParams);//, existingDynamicPx
	...
	void setLastRigidTransform(const Eigen::Matrix4f& lastRigidTransform) {
        ...
        m_hashParams.m_rigidTransform.m11 = lastRigidTransform(0,0);
        m_hashParams.m_rigidTransform.m12 = lastRigidTransform(0,1);
        m_hashParams.m_rigidTransform.m13 = lastRigidTransform(0,2);
        m_hashParams.m_rigidTransform.m14 = lastRigidTransform(0,3);
        m_hashParams.m_rigidTransform.m21 = lastRigidTransform(1,0);
        m_hashParams.m_rigidTransform.m22 = lastRigidTransform(1,1);
        m_hashParams.m_rigidTransform.m23 = lastRigidTransform(1,2);
        m_hashParams.m_rigidTransform.m24 = lastRigidTransform(1,3);
        m_hashParams.m_rigidTransform.m31 = lastRigidTransform(2,0);
        m_hashParams.m_rigidTransform.m32 = lastRigidTransform(2,1);
        m_hashParams.m_rigidTransform.m33 = lastRigidTransform(2,2);
        m_hashParams.m_rigidTransform.m34 = lastRigidTransform(2,3);
        m_hashParams.m_rigidTransform.m41 = lastRigidTransform(3,0);
        m_hashParams.m_rigidTransform.m42 = lastRigidTransform(3,1);
        m_hashParams.m_rigidTransform.m43 = lastRigidTransform(3,2);
        m_hashParams.m_rigidTransform.m44 = lastRigidTransform(3,3);
	...
	void setLastRigidTransformAndCompactify(const Eigen::Matrix4f& lastRigidTransform) {
		setLastRigidTransform(lastRigidTransform);
		compactifyHashEntries();
	}
	...
	void reset() {
		//static bool tanFirst = false;

		m_numIntegratedFrames = 0;

		m_hashParams.m_rigidTransform.setIdentity();
		m_hashParams.m_rigidTransformInverse.setIdentity();
		//printf("m_numOccupiedBlocks:%d\n", m_hashParams.m_numOccupiedBlocks);
		//m_hashParams.m_numOccupiedBlocks = m_hashParams.m_numOccupiedBlocks - m_blockOffset > 0 ? m_hashParams.m_numOccupiedBlocks - m_blockOffset : 0;
		...
		//else{
			unsigned int temp = 0;
			MLIB_CUDA_SAFE_CALL(cudaMemcpy(&temp, m_hashData.d_heapCounter, sizeof(unsigned int), cudaMemcpyDeviceToHost));
			printf("test:%d \n", temp);
		//	m_hashData.free();
		//	m_hashData.allocate(m_hashParams);
			...
			//printf("test:%d \n", temp);
		//}
		resetCUDA(m_hashData, m_hashParams, 0);
		MLIB_CUDA_SAFE_CALL(cudaMemcpy(&temp, m_hashData.d_heapCounter, sizeof(unsigned int), cudaMemcpyDeviceToHost));
		printf("test:%d \n", temp);
		printf("%d, %d", m_hashParams.m_hashNumBuckets, HASH_BUCKET_SIZE);
	}
...
private:
    ...
    /*void insertMaskMap(const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams) {
        //Start Timing
        if(GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.start(); }

        *//*uchar* mmm = new char[320*240];
        cudaMemcpy(mmm, depthCameraData.d_maskData, sizeof(uchar)*320*240, cudaMemcpyHostToDevice);
        ...
        insertMaskMapCUDA(m_hashData, m_hashParams, depthCameraData, depthCameraParams);
    ...
    void calculateSDFMapFromVoxel(float* sdfMap, const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams)
    {
        if(GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.start(); }

        calculateSDFMapFromVoxelCUDA(sdfMap, m_hashData, m_hashParams, depthCameraData, depthCameraParams);

        // Stop Timing
        if (GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.stop(); TimingLogDepthSensing::totalTimeIntegrate += m_timer.getElapsedTimeMS(); TimingLogDepthSensing::countTimeIntegrate++; }
    }
	void gen_voxel_image(uchar3* img, const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams, uchar* mask_gpu)
	{
		if(GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.start(); }

		gen_imageCUDA(img, m_hashData, m_hashParams, depthCameraData, depthCameraParams, mask_gpu);

		// Stop Timing
		if (GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.stop(); TimingLogDepthSensing::totalTimeIntegrate += m_timer.getElapsedTimeMS(); TimingLogDepthSensing::countTimeIntegrate++; }
	}
    void gen_virtual_voxel_image(uchar3* img, const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams)
    {
        if(GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.start(); }

        gen_virtual_Image_CUDA(img, m_hashData, m_hashParams, depthCameraData, depthCameraParams);
        // Stop Timing
        if (GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.stop(); TimingLogDepthSensing::totalTimeIntegrate += m_timer.getElapsedTimeMS(); TimingLogDepthSensing::countTimeIntegrate++; }
    }
    void removeExistingDynamicPx2DepthMap(const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams)
    {
        if(GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.start(); }

        removeExistingDynamicPx2DepthMapCUDA(m_hashData, m_hashParams, depthCameraData, depthCameraParams);

        // Stop Timing
        if (GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.stop(); TimingLogDepthSensing::totalTimeIntegrate += m_timer.getElapsedTimeMS(); TimingLogDepthSensing::countTimeIntegrate++; }
    }
	void integrateDepthMap(const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams) {//, uint* existingDynamicPx
		//Start Timing
		if(GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.start(); }

		integrateDepthMapCUDA(m_hashData, m_hashParams, depthCameraData, depthCameraParams);//, existingDynamicPx

		// Stop Timing
		if (GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.stop(); TimingLogDepthSensing::totalTimeIntegrate += m_timer.getElapsedTimeMS(); TimingLogDepthSensing::countTimeIntegrate++; }
	}
    /*void integrateDepthMapAndConsistency(const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams, const float* consistency_optical_flow_px) {
        //Start Timing
        if(GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.start(); }

        integrateDepthMapAndConsistencyCUDA(m_hashData, m_hashParams, depthCameraData, depthCameraParams, consistency_optical_flow_px);

        // Stop Timing
        if (GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.stop(); TimingLogDepthSensing::totalTimeIntegrate += m_timer.getElapsedTimeMS(); TimingLogDepthSensing::countTimeIntegrate++; }
    }*/

	void deIntegrateDepthMap(const DepthCameraData& depthCameraData, const DepthCameraParams& depthCameraParams) {//, uint* existingDynamicPx
		//Start Timing
		if (GlobalAppState::get().s_timingsDetailledEnabled) { cutilSafeCall(cudaDeviceSynchronize()); m_timer.start(); }

		deIntegrateDepthMapCUDA(m_hashData, m_hashParams, depthCameraData, depthCameraParams);//, existingDynamicPx
