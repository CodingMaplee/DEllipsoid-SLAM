...
public:
  ...
  /*  mat4f getIntegratedTransform(uint frameIdx)
    {
        return
    }*/
	...
	void saveOptimizedTransform(const float4x4* d_trajectory, unsigned int numFrames, int updateTimer, int fullNum, const float4x4* d_localTrajectory);
    ...
    bool getMaskList ( mat4f& trans, unsigned int frameIdx);
...
private:
	...
	std::vector<mat4f> m_optmizedTransformsCopy;
