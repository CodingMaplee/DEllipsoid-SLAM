...
Sophus::SE3f Tracking::GrabImageRGBD(const std::vector<Detection::Ptr>& detections, const uchar* personMask, const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp, string filename)
{
    current_frame_idx_ = (current_frame_idx_ + 1) % (std::numeric_limits<size_t>::max()-1);
    mImGray = imRGB;
    cv::Mat optimizedMat_src1 ;
    cv::Mat optimizedMat_src2 ;
    cv::Mat imDepth = imD;
    ...
    if((fabs(mDepthMapFactor-1.0f)>1e-5) || imDepth.type()!=CV_32F)
        imDepth.convertTo(imDepth,CV_32F,mDepthMapFactor);//read depth

//for  ..  tr in objectTracks_, if objectTracks_[i] is dynamic, then detection[j] be set dynamic.
//if no assigned detection, then remove bbox or eli??

   if (mSensor == System::RGBD)
        mCurrentFrame = Frame(mImGray, imDepth, timestamp, mpORBextractorLeft, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, detections, mvDynamicCatId, personMask, imRGB, mpCamera);//

        //cv::imwrite("../img/"+ to_string(timestamp)+".jpg",mCurrentFrame.im_Rgb);
...
//        mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
    ...
    current_frame_good_detections_ = detections;
    std::chrono::steady_clock::time_point start = std::chrono::steady_clock::now();
    Track();
    std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
    std::chrono::duration<double> diff = end - start;
    std::cout << "Time used:  " << diff.count()*1000 << " ms";
    double MIN_2D_IOU_THRESH = 0.2;
    double MIN_3D_IOU_THRESH = 0.3;
    int TIME_DIFF_THRESH = 30;
    int person_cat = mpAtlas->GetMapLabelValue().find("person")->second;
    int box_cat = mpAtlas->GetMapLabelValue().find("box")->second;

    //int balloon_cat = mpAtlas->GetMapLabelValue().find("balloon")->second;
    if (mState == Tracking::OK) {
        KeyFrame *kf = mpLastKeyFrame;
        if (!createdNewKeyFrame_)
            kf = nullptr;
        Eigen::Matrix<float, 3, 4> Rt = mCurrentFrame.GetPose().matrix3x4();  //cw  wc()
        Eigen::Matrix<double, 3, 4> Rt_double = Rt.cast<double>();
        Matrix34d P;
        P = K_ * Rt_double;
        ////////////////////////process existing objects///////////////////////
        std::unordered_map<ObjectTrack::Ptr, BBox2> proj_bboxes;
        BBox2 img_bbox(0, 0, 640, 480);
            ...
            if (tr->GetStatus() == ObjectTrackStatus::INITIALIZED ||
                ...
                for (auto it : proj_bboxes) {
                    if (it.first != tr && bboxes_iou(it.second, bb) > 0.9) {
                        ...
                        double z2 = Rt_double.row(2).dot(c2.homogeneous());
                        if (z < z2) {
        ...
        //should be compared to static or dynamic?
        std::vector<ObjectTrack::Ptr> possible_tracks;
        for (auto tr : objectTracks_) {
            auto bb = tr->GetLastBbox();
            //TODO 30  bboxes_intersection(bb, img_bbox) >= 0.3 * bbox_area(bb)
            if (tr->GetLastObsFrameId() + 600 >= current_frame_idx_ &&
        ...
        std::vector<int> assign_test(current_frame_good_detections_.size());
        vector<Eigen::Vector3d> personMapPoints;
        for(int i = 0; i < current_frame_good_detections_.size(); i++)
        {
            Detection::Ptr det = current_frame_good_detections_[i];
            if (det->category_id == person_cat ){
//                Eigen::Vector4d box_curr = det->bbox;
//                std::vector<cv::KeyPoint> keyPoint_curr = mCurrentFrame.GetKeyPoints_curr(imRGB);
               //std::vector<cv::KeyPoint> personKeyPoints = mCurrentFrame.GetFeaturesInBox(box_curr,keyPoint_curr);
                personMapPoints = mCurrentFrame.GetMapPoints(det->mask, Rt_double, imDepth);
                //continue;
            }
            else{
                ...
                {
                    ObjectTrack::Ptr tr = possible_tracks[j];
                    auto bb = tr->GetLastBbox();
                    if(possible_tracks[j]->category_id_ == det->category_id )
                    {
                        Eigen::Matrix4d RT =Eigen::Matrix4d::Identity();
                        RT.block<3,4>(0,0) = Rt_double;
//                        //TODO  question "Dynamic or static"
                        int pointInEllipsoid=0;
                        if (tr->category_id_ == box_cat && current_frame_idx_>=220){
                            //cout<<tr->category_id_<<endl;
                            tr->SetIsDynamic();
                        }
                        //current_frame_idx_<=300
//                        else if (tr->category_id_ == box_cat && current_frame_idx_<=300)
//                        {
//                            tr->SetIsStatic();
//                        }
//                        if (!personMapPoints.empty() &&  tr->GetStatus() == ObjectTrackStatus::IN_MAP){
//                            const Ellipsoid ellipsoid =  tr->GetMapObject()->GetEllipsoid();
//                            for (int k=0; k<personMapPoints.size(); k++){
//                                if (ellipsoid.IsInside(personMapPoints[k])) {
//                                   pointInEllipsoid++;
//                               }
//                            }
//                            //cout<<"category_id: "<<tr->category_id_<<" nums: "<<pointInEllipsoid<<endl;
//                        }
                        //person point number
//                        if(pointInEllipsoid > DYNAMIC_PERSON_POINT_NUMBER && !tr->IsDynamic())
//                        {
//                            mvDynamicCatId.insert(det->category_id);
//                            //cout <<  "current_frame_idx_: " << current_frame_idx_ << endl;
//                            //cout<<"Dynamic_id:"<<tr->category_id_<<endl;
//                            tr->SetIsDynamic();
//                        }
        ...
        for (size_t di = 0; di < current_frame_good_detections_.size(); ++di) {
            ...
            if (det->category_id == person_cat ) continue;
            auto assigned_track_idx = assign_test[di];
            //mpAtlas->optImg = imRGB;
            if(current_frame_idx_ == 1){
                mpAtlas->optImg = imRGB;
            }
            optimizedMat_src1 = mpAtlas->optImg;
            optimizedMat_src2 = mpAtlas->optImg;
            //optimizedMat_src1 =imRGB;
            //optimizedMat_src2 =imRGB;
            if (assign_test[di] == -1) {
                auto tr = ObjectTrack::CreateNewObjectTrack(det->category_id, det->bbox, det->score, Rt_double, imD,
                                                            det->centerPoint_cam,
                                                            current_frame_idx_, this, kf);
                if (tr->CenterPoint_.front().z()>=1e-3){
                    //std::cout << "create new track " << tr->category_id_ << "\n";
                    //first cam pos
//                    if(tr->category_id_ == box_cat)
//                    {
//                        //cout << " Rt_double: " << Rt_double  << endl;
//                    }
                ...
                associated_track->AddDetection(det->bbox, det->score, det->centerPoint_cam, Rt_double, current_frame_idx_, kf);
                cv::Scalar color_opt_after = cv::Scalar(det->category_id*5,det->category_id*5,det->category_id*5);
                if (associated_track->IsDynamic()) {
                    cv::Scalar color_opt_previous = cv::Scalar(det->category_id*5, det->category_id*5, det->category_id*5);
                    bool optTrue = associated_track->TrackDynamic();
                    mpFrameDrawer->Update(this);
                    if (optTrue) mpAtlas->optImg = mpFrameDrawer->DrawProjections(optimizedMat_src1, color_opt_previous, associated_track->category_id_);
                }
                else{
                    mpAtlas->optImg = mpFrameDrawer->DrawProjections(optimizedMat_src2, color_opt_after, associated_track->category_id_);
                    if (kf && associated_track->GetStatus() == ObjectTrackStatus::IN_MAP){
//                        if(associated_track->category_id_==6 && current_frame_idx_>=240){
//                            continue;
//                        }
                         if (local_object_mapper_ )//&& current_frame_idx_ <= 300
                        {
                            local_object_mapper_->InsertModifiedObject(associated_track->GetMapObject());
//                            if (associated_track->category_id_==6 && current_frame_idx_<=200)
//                                local_object_mapper_->InsertModifiedObject(associated_track->GetMapObject());
//                            else{
//                                local_object_mapper_->InsertModifiedObject(associated_track->GetMapObject());
//                            }
                ...
                //cv::imwrite("../img2/" + std::to_string(current_frame_idx_) + ".png", mpAtlas->optImg);
            }
        ...
        if (!mbOnlyTracking) {
            for (auto& tr : objectTracks_) {
                if (tr->GetLastObsFrameId() == current_frame_idx_) {
                    if ((tr->GetNbObservations() > 2 && tr->GetStatus() == ObjectTrackStatus::ONLY_2D ) ||
                        (tr->GetNbObservations() % 2 == 0 && tr->GetStatus() == ObjectTrackStatus::INITIALIZED)) {
                        //cout<<"ReconstructFromCenter!!!!"<<endl;
                        //cout<<"ReconstructFromCenter: "<<tr->category_id_<<endl;
                        bool status_rec= false;
                        // box
                        if (tr->category_id_!=-1)
                           status_rec = tr->ReconstructFromCenter(); // try to reconstruct and change status to INITIALIZED if success
                        if(status_rec){
                            //cout<<"ReconstructFromCenter end: "<<tr->category_id_<<endl;
                            ///////////static hidden//////////////////
                            //if (240<=current_frame_idx_&&current_frame_idx_<=330)  continue;
                            tr->OptimizeReconstruction(mpAtlas);
                            cout<<"OptimizeReconstruction "<<endl;
                        }
                    }
                }
                if (tr->GetNbObservations() >= 10 && (tr->GetStatus() == ObjectTrackStatus::INITIALIZED )) {
                    tr->OptimizeReconstruction(mpAtlas);
                    ...
                    if (checked){
                        ...
                        tr->SetIsBad();
                        mvDynamicCatId.erase(tr->category_id_);
...
void Tracking::RemoveTrack(ObjectTrack::Ptr track)
{
    if (track->GetMapObject())
        mpAtlas->EraseMapObject(track->GetMapObject());
    objectTracks_.remove(track);
}
void Tracking::RemoveMapObject(ObjectTrack::Ptr track){
    if (track->GetMapObject())
        mpAtlas->EraseMapObject(track->GetMapObject());
}
...
void Tracking::Track()
{
        ...
        if(!mbOnlyTracking)
        {
            ...
            else
            {
                ...
                if (mState == RECENTLY_LOST)
                {
                    ...
                    {
                        if (mpSystem->GetRelocalizationMode() == mpSystem->enumRelocalizationMode::RELOC_POINTS) {
                            std::cout << "Relocalize with points.\n";
                            bOK = Relocalization();
                        }
                        else if (mpSystem->GetRelocalizationMode() == mpSystem->enumRelocalizationMode::RELOC_OBJECTS) {
                            std::cout << "Relocalize with objects.\n";
                            bOK = RelocalizationFromObjects(false);
                        }
                        else if (mpSystem->GetRelocalizationMode() == mpSystem->enumRelocalizationMode::RELOC_OBJECTS_POINTS) {
                        ...
                        if(mCurrentFrame.mTimeStamp-mTimeStampLost>30.0f && !bOK)
                        //30   mState = LOST init
                        {
                            cout << "mState LOST init" << endl;
                            mState = LOST;
                            Verbose::PrintMess("Track Lost...", Verbose::VERBOSITY_NORMAL);
                            bOK=false;
                        }
    ...
    if(mState==OK || mState==RECENTLY_LOST)
    {
        ...
        else
        {
            ...
            mlRelativeFramePoses.push_back(Sophus::SE3f());
...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        ...
        // Initialization Create KeyFrame
        ...
        Verbose::PrintMess("New Map11 created with " + to_string(mpAtlas->MapPointsInMap()) + " points", Verbose::VERBOSITY_QUIET);
...
void Tracking::CheckReplacedInLastFrame()
{
    for(int i =0; i<mLastFrame.N; i++)
    {
        ...
        if(pMP)
        {
            ...
            //std::cout<<"mTrackProjX:"<<mLastFrame.mvpMapPoints[i]->mTrackProjX<<"mTrackProjyY:"<<mLastFrame.mvpMapPoints[i]->mTrackProjY<<"mTrackDepth"<<mLastFrame.mvpMapPoints[i]->mTrackDepth<<std::endl;
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    //std::cout << " TrackReferenceKeyFrame mLastFrame.mTcw:  "<< mLastFrame. << std::endl;
...
bool Tracking::TrackDynamicReferenceKeyFrame()
{
    // Compute Bag of Words vector
    mCurrentFrame.ComputeBoW();
    // We perform first an ORB matching with the reference keyframe
    ...
    int nmatches = matcher.SearchByBoW(mpDynamicReferenceKF,mCurrentFrame,vpMapPointMatches);
...
bool Tracking::RelocalizationFromObjects(bool use_points) {
    ...
    auto objects = mpAtlas->GetAllMapObjects();
    ...
    int best_index=-1;
    std::vector<Matrix34d, Eigen::aligned_allocator<Matrix34d>>poses = std::vector<Matrix34d, Eigen::aligned_allocator<Matrix34d>>();
    std::vector<double> scores =  std::vector<double>();
    std::vector<std::vector<std::pair<size_t, size_t>>> used_pairs = std::vector<std::vector<std::pair<size_t, size_t>>>();
    std::vector<std::vector<std::pair<size_t, size_t>>> used_inliers = std::vector<std::vector<std::pair<size_t, size_t>>>();

    ORB_SLAM3::solveP3P_ransac(ellipsoids, ellipsoids_categories, bboxes, bboxes_categories, K_, 0.2, best_index, poses, scores, used_pairs, used_inliers);
...
//    });
    ...
    for (int i = 0; i < indices.size() && scores[indices[i]] < bboxes.size(); ++i) {
        ...
        Rt.at<float>(3, 3) = 1.0;
        Eigen::Matrix<float,3,4> Rt_34 = Rt_est.cast<float>();
        Eigen::Matrix<float,4,4> Rt_44 = Eigen::Matrix<float,4,4>::Identity();
        Rt_44.block<3,4>(0,0) = Rt_34;
        mCurrentFrame.SetPose(Sophus::SE3f(Rt_44));
        ...
        auto map_points = mpAtlas->GetAllMapPoints();
        ORBmatcher matcher(0.9, true);
