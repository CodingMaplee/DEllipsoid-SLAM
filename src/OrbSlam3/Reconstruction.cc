                                  ...
                                  const std::vector<Matrix34d, Eigen::aligned_allocator<Matrix34d>>& projections)
{
    ...
    //DLT
    for (int i = 0; i < n; ++i) {
//        Eigen::Matrix<double,3,4> RT = K.inverse() * projections[i];
//        Eigen::Matrix<double,4,4>RT_inv;
        //RT_inv.block<3,4>(0,0)=RT;
        // RT_inv(3,0)=0;
        //RT_inv(3,1)=0;
        //RT_inv(3,2)=0;
        //RT_inv(3,3)=1;
        //RT_inv = RT_inv.inverse();
        //Eigen::Matrix<double,3,4>projection2 = projection_inv.block<3,4>(0,0);
        //Eigen::Vector4d centerPoint4d = Eigen::Vector3d( points[i][0],points[i][1],1).transpose() * projection2;
        //std::cout<<"points x="<< points[i][0]<<" y="<< points[i][1]<<std::endl;
        //std::cout<<"point3d x="<<centerPoint4d[0]<<" y="<<centerPoint4d[1]<<" z="<<centerPoint4d[2]<<std::endl;
                                ...
                                const Eigen::Matrix3d& K)
{
    ...
    Eigen::Vector3d center = CenterPoint3ds.front();
//    if (center.x() ==0){
...
//    }
    ...
    for (size_t i = 0; i < n; ++i) {
        //std::cout<<"center0: x="<<center.x()<<" y="<<center.y()<<" z="<<center.z()<<std::endl;
        //std::cout<<"center1: x="<<center.homogeneous().x()<<" y="<<center.homogeneous().y()<<" z="<<center.homogeneous().z()<<std::endl;
        Eigen::Vector3d X_cam = Rts[i] * center.homogeneous();
        //std::cout<<Rts[i]<<std::endl;
        //std::cout<<"points2d i:"<<i<<" x="<<points2d[i].x()<<" y="<<points2d[i].y()<<std::endl;
       // std::cout<<"center2: x="<<center.homogeneous().x()<<" y="<<center.homogeneous().y()<<" z="<<center.homogeneous().z()<<std::endl;

        // Reconstruction failed if the object is behind a camera
        if (X_cam.z() < 0) {
            //std::cout<<"X_cam.z"<<X_cam.z()<<std::endl;
            //std::cerr << "Reconstruction failed: z is negative" << std::endl;
            return {false, Ellipsoid()};
        }
        Eigen::Vector3d X_img = K * X_cam;
        //std::cout<<"X_cam: x="<<X_cam.x()<<" y="<<X_cam.y()<<" z="<<X_cam.z()<<std::endl;
