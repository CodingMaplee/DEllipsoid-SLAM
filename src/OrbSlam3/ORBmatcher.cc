...
int ORBmatcher::SearchDynamicByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
{
    const vector<MapPoint*> vpMapPointsKF = pKF->GetMapPointMatches();
    ...
    int ORBmatcher::SearchByProjectionAllPoints(Frame &CurrentFrame, const std::vector<MapPoint*>& points, const float th , const int ORBdist)
    {
        int nmatches = 0;
        const Eigen::Matrix<float,3,4> Rt_matrix  = CurrentFrame.GetPose().matrix3x4();
        cv::Mat Rt_cv(4, 4, CV_32F, 0.0);
        for (size_t i = 0; i < 3; ++i)
            for (size_t j = 0; j < 4; ++j)
                Rt_cv.at<float>(i, j) = Rt_matrix(i, j);
        Rt_cv.at<float>(3, 3) = 1.0;
        const cv::Mat Rcw = Rt_cv.rowRange(0,3).colRange(0,3);
        const cv::Mat tcw = Rt_cv.rowRange(0,3).col(3);
        ...
        for(size_t i=0, iend=points.size(); i<iend; i++)
        {
            ...
            if(pMP)
            {
                ...
                {
                    //Project
                    Eigen::Vector3f x3Dw_eigen =  pMP->GetWorldPos();
                    cv::Mat x3Dw(3, 1, CV_32F, 0.0);
                    x3Dw.at<float>(0,0) = x3Dw_eigen.x();
                    x3Dw.at<float>(1,0) = x3Dw_eigen.y();
                    x3Dw.at<float>(2,0) = x3Dw_eigen.z();
