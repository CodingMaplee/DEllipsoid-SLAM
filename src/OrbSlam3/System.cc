    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
  ...
  mpLocalMapper = new LocalMapping(this, mpAtlas, mSensor==MONOCULAR || mSensor==IMU_MONOCULAR,
    ...
    if (use_objects_in_local_BA_ <= 1) {
        local_object_mapper_ = new LocalObjectMapping(mpAtlas, mpTracker);
        local_object_mapping_ = new thread(&ORB_SLAM3::LocalObjectMapping::Run, local_object_mapper_);
    }
...
void System::GetImageSize(uint& width, uint& height)
{
    settings_->getImageSize(width, height);
}
double System::GetDepthFactor()
{
    double factor = settings_->depthMapFactor();
    return factor;
}
void System::SetMapLabelValue(map<string, int> label_value){
    mpAtlas->SetMapLabelValue(label_value);
}
Eigen::Matrix4f System::GetDepthIntrinsics()
{
    float fx = settings_->camera1()->getParameter(0);
    float fy = settings_->camera1()->getParameter(1);
    float cx = settings_->camera1()->getParameter(2);
    float cy = settings_->camera1()->getParameter(3);
...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const std::vector<Detection::Ptr>& detections, cv::Mat& featureImg, const uchar* personMask, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    if (mSensor == System::IMU_RGBD)
        for(size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++)
            mpTracker->GrabImuData(vImuMeas[i_imu]);
    Sophus::SE3f Tcw = mpTracker->GrabImageRGBD(detections, personMask, imToFeed, imDepthToFeed, timestamp, filename);
    featureImg = mpFrameDrawer->DrawFrame(1.0);
    unique_lock<mutex> lock2(mMutexState);
    mTrackingState = mpTracker->mState;
    mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;
    mTrackedKeyPointsUn = mpTracker->mCurrentFrame.mvKeysUn;
...
std::vector<Sophus::SE3f> System::ExtractTrajectoryTUM()
{
    std::vector<Sophus::SE3f> allTrajectories;
    //std::cout<<"11"<<std::endl;
    vector<KeyFrame*> vpKFs = mpAtlas->GetAllKeyFrames();
    sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
    //std::cout<<"22"<<std::endl;
    std::cout<<vpKFs.size()<<std::endl;
    Sophus::SE3f Two;
    if(vpKFs.size() > 0)
    {
                ...
                lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        //Sophus::SE3f Twc = Tcw.inverse();  //camera -> world
        allTrajectories.push_back(Twc);
    }
    return allTrajectories;

}

unordered_set<int> System::ObjectInMap()
{

    vector<MapObject*> mapObj = mpAtlas->GetAllMapObjects();
    unordered_set<int> category_ellipsoid_id;
    for(int i = 0; i < mapObj.size(); i++)
    {
        category_ellipsoid_id.insert(mapObj[i]->GetTrack()->category_id_);
    }
    return category_ellipsoid_id;
}
cv::Mat System::GetOptImg(){
    return mpAtlas->optImg;
}
void System::SetOptImg(cv::Mat img){
    mpAtlas->optImg = img;
}
void System::SaveTrajectoryTUM(const string &filename)
{
    cout << endl << "Saving camera trajectory222 to " << filename << " ..." << endl;
    if(mSensor==MONOCULAR)
    {
...
void System::SaveKeyFrameInstance_result()
{
    cout << endl << "Saving keyframe Instance_result to " << " ..." << endl;
    vector<KeyFrame*> vpKFs = mpAtlas->GetAllKeyFrames();
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        if(pKF->isBad())
            continue;
        int * mask = NULL;
        //pKF->GetInstanceResult(mask);
        cv::Mat instanceMat = cv::Mat(480, 640, CV_8UC1);
        for (int i = 0; i < 480; i++)
        {
            for(int j = 0;j < 640;j++)
            {
        ...
        cv::imwrite("../KeyFrame_result/" + std::to_string(pKF->mTimeStamp) + ".png", instanceMat);
    }
