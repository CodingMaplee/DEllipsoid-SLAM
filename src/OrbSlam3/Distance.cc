...
{
    std::tuple<Eigen::Vector2d, Eigen::Matrix2d> tu_1;
    std::tuple<Eigen::Vector2d, Eigen::Matrix2d> tu_2;

    tu_1 = ell1.AsGaussian();
    tu_2 = ell2.AsGaussian();

    Eigen::Matrix2d sigma11 = std::get<1>(tu_1).sqrt();
    Eigen::Matrix2d s121 = sigma11 * std::get<1>(tu_2) * sigma11;
    Eigen::Matrix2d sigma121 = s121.sqrt();

    double d = (std::get<0>(tu_1)-std::get<0>(tu_2)).squaredNorm() + (std::get<1>(tu_1) + std::get<1>(tu_2) - 2 * sigma121).trace();
    return d;
}
...
double gaussian_bhattacharrya_2d(const Ellipse& ell1, const Ellipse& ell2)
{
    ...
    Eigen::Matrix2d G = (std::get<1>(tu_1) + std::get<1>(tu_2)) * 0.5;
    Eigen::Matrix2d G_inv = G.inverse();
    Eigen::Vector2d mu_diff = std::get<0>(tu_1) - std::get<0>(tu_2);
    Eigen::Vector2d temp = G_inv * mu_diff;
    double dist = 0.125 * mu_diff.dot(temp);
    dist += 0.5 * std::log(G.determinant() / std::sqrt(std::get<1>(tu_1).determinant() * std::get<1>(tu_2).determinant()));
    return dist;
}
...
std::tuple<bool, BBox2> find_on_image_bbox(const Ellipse& ell, int width, int height)
{
    ...
    if (bb[2] < 0 || bb[3] < 0 || bb[0] >= width || bb[1] >= height)
        return std::tuple<bool, BBox2>(false, bb);
    ...
    if (count == 0) {
        BBox2 bbox(0, 0, width-1, height-1);
        return std::tuple<bool, BBox2>(true, bbox);
    }
    BBox2 bbox2(xmin, ymin, xmax, ymax);
    return std::tuple<bool, BBox2>(true, bbox2);
}
