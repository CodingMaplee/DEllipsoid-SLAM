...
#include <thread>
#include <../include/OrbSlam3/CameraModels/Pinhole.h>
#include <../include/OrbSlam3/CameraModels/KannalaBrandt8.h>
...
#endif
//    thread threadLeft(&Frame::ExtractORB,this,0,imLeft,0,0,dynamicMask);
//    thread threadRight(&Frame::ExtractORB,this,1,imRight,0,0,dynamicMask);
//    threadLeft.join();
//    threadRight.join();
#ifdef REGISTER_TIMES
...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, const std::vector<Detection::Ptr>& detections, set<int> &mvDynamicCatId, const uchar* personMask, const cv::Mat &imRgb, GeometricCamera* pCamera, Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()), mK_(Converter::toMatrix3f(K)),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
     mImuCalib(ImuCalib), mpImuPreintegrated(NULL), mpPrevFrame(pPrevF), mpImuPreintegratedFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbIsSet(false), mbImuPreintegrated(false),
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false),im_Rgb(imRgb)
{
    // Frame ID
    mnId=nNextId++;
...
//    for (int i=0;i<mvDynamicCatId.size();i++){
//        cout<<"mvDynamicCatId: "<<mvDynamicCatId[i]<<endl;
//    }
    for (size_t i(0); i < mvKeys.size(); ++i)
    {
        uchar person_val = personMask[(uint)(mvKeys[i].pt.y + 0.5) * FRAME_WIDTH + (uint)(mvKeys[i].pt.x + 0.5)];
        uchar mat_val = 0;
        uchar dynamic_val = 0;
        for (int k=0; k<detections.size(); k++){
            int cat_id =detections[k]->category_id;
//            if (find(mvDynamicCatId.begin(), mvDynamicCatId.end(), cat_id)!= mvDynamicCatId.end()){
//                mat_val = detections[k]->mask[(uint)(mvKeys[i].pt.y + 0.5) * FRAME_WIDTH + (uint)(mvKeys[i].pt.x + 0.5)];
//                if (mat_val == 255){
//                    dynamic_val = 255;
//                }
//            }
//          TODO:box features
            if (cat_id == 3){
                dynamic_val =  detections[k]->mask[(uint)(mvKeys[i].pt.y + 0.5) * FRAME_WIDTH + (uint)(mvKeys[i].pt.x + 0.5)];
            }
        }
        if (person_val == 0 && dynamic_val == 0)
        {
...
static bool cmpx(cv::KeyPoint a , cv::KeyPoint b){
    return a.pt.x < b.pt.x;
}

static bool cmpy(cv::KeyPoint a , cv::KeyPoint b){
    return a.pt.y < b.pt.y;
}
vector<size_t> Frame::GetFeaturesInMask(uchar* mask, std::vector<cv::KeyPoint> keyPoint_curr, Eigen::Vector4d bbox)
{
    cout<<"bbox: "<<bbox<<endl;
    vector<size_t> result;
    //int len = int(bbox.w() - bbox.y()) * 640;

    //cout<<"len "<<len<<endl;
    std::vector<cv::KeyPoint> mask_cat_x;
    std::vector<cv::KeyPoint> mask_cat_y;
    int j = 0;
    cout<<"bbox.y:"<< bbox.y()<<endl;
    for(int i = int(bbox.y()+5) * 640; i < (bbox.w()-5)*640; i++)
    {
        cout<<i<<endl;
        if(mask[i] == 255)
        {
            ...
            kp.pt.x =int(i % 640);
            kp.pt.y =int(i / 640);
            mask_cat_x.push_back(kp);
            mask_cat_y.push_back(kp);
        }
    }
    sort(mask_cat_x.begin(),mask_cat_x.end(), cmpx);
    sort(mask_cat_y.begin(),mask_cat_y.end(), cmpy);
    int minx =mask_cat_x[0].pt.x;
    int maxx =mask_cat_x[mask_cat_x.size()-1].pt.x;
    int miny =mask_cat_y[0].pt.y;
    int maxy =mask_cat_y[mask_cat_y.size()-1].pt.y;



    //cout<<"len "<<len<<endl;
    for (int i=0;i<keyPoint_curr.size();i++){
        bool left = false;
        bool right = false;
        if(!(keyPoint_curr[i].pt.x < minx || keyPoint_curr[i].pt.y < miny))
            left = true;
        if(!(keyPoint_curr[i].pt.x > maxx || keyPoint_curr[i].pt.y > maxy))
            right = true;
        if(left && right)
            result.push_back(i);
    }
...
std::vector<cv::KeyPoint> Frame::GetKeyPoints_curr(cv::Mat im){
...
std::vector<cv::KeyPoint> Frame::GetFeaturesInBox(Eigen::Vector4d box, std::vector<cv::KeyPoint> keyPoint_curr){
    std::vector<cv::KeyPoint> result;
    for (int i=0;i<keyPoint_curr.size();i++){
      if (box.x()<=keyPoint_curr[i].pt.x && keyPoint_curr[i].pt.x<=box.z() && box.y()<=keyPoint_curr[i].pt.y && keyPoint_curr[i].pt.y<=box.w()){
          result.push_back(keyPoint_curr[i]);
      }
    }
    return result;
}
vector<Eigen::Vector3d> Frame::GetMapPoints(uchar* personMask, Eigen::Matrix<double, 3, 4> Rt_double, cv::Mat imDepth)
{
    ...
    {
//        if (personMask[i]!=0){
//        cout<<"personMask:"<<personMask[i]<<endl;
//        }
        if(personMask[i] == 255)
        {
            int u = int(i % 640);
...
//                Eigen::Vector3d keyPoint = Eigen::Vector3d(u,v,1.0);
                Eigen::Vector4d KeyPoint_cam =  Eigen::Vector4d (depth * (u - cx) / fx,
                                                                 depth * (v - cy) / fy, depth, 1.0);
                Eigen::Matrix4d RT =Eigen::Matrix4d::Identity();
                RT.block<3,4>(0,0) = Rt_double;
                Eigen::Vector4d mapPoint4d = RT.inverse() * KeyPoint_cam;
                Eigen::Vector3d mapPoint = Eigen::Vector3d (mapPoint4d.x(),mapPoint4d.y(),mapPoint4d.z());
...
//std::vector<cv::KeyPoint> Frame::GetFeaturePoint2d(){
//    for (int i; i < point2d_index.size(); i++){
//        cv::Point2f cvP1(mvKeys[point2d_index[i]].pt.x,mvKeys[point2d_index[i]].pt.y );
//    }
//
//}
...
bool Frame::ProjectPointDistort(MapPoint* pMP, cv::Point2f &kp, float &u, float &v)//projectPointDistort
{
