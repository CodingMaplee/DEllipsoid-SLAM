...
void Atlas::SetMapLabelValue(map<string, int> label_value){
    map_label_value = label_value;
}
map<string, int> Atlas::GetMapLabelValue(){
    return map_label_value;
}
...
void Atlas::AddMapObject(MapObject *obj)
{
    unique_lock<mutex> lock(mMutexAtlas);
    map_objects_.insert(obj);
}
void Atlas::EraseMapObject(MapObject *obj)
{
    unique_lock<mutex> lock(mMutexAtlas);
...
std::vector<MapPoint*> Atlas::CollectMapPointInObject(MapObject *obj)
{
    unique_lock<mutex> lock(mMutexAtlas);
    cout<<"id: "<<obj->GetTrack()->category_id_<<endl;
    vector<MapPoint*> mapPointsAll = mpCurrentMap->GetAllMapPoints();
    //unsigned long ReferenceMapId = mpCurrentMap->;
    //cout<<"ReferenceMapId: "<<ReferenceMapId<<endl;
    vector<MapPoint*> result;
    for(int i=0;i<mapPointsAll.size();i++){
      if (mapPointsAll[i]){
         Eigen::Vector3f point = mapPointsAll[i]->GetWorldPos();
          Eigen::Vector3d point2 = Eigen::Vector3d (point.x(),point.y(),point.z());
         if (obj->GetEllipsoid().IsInside(point2)){
             //cout<<"point3d: x= "<<point2.x()<<" y="<<point2.y()<<" z="<<point2.z()<<endl;
             result.push_back(mapPointsAll[i]);
         }
      }
    }
    return result;
