...
void TrajectoryManager::generateUpdateLists()
{
    ...
    {
        if ( left->type == TrajectoryFrame::Integrated && right->type != TrajectoryFrame::Integrated )
        {
            return true;
        }
        if ( left->type != TrajectoryFrame::Integrated )
        {
            return false;    //needs a strict less than comparison function // && right->type == TrajectoryFrame::Integrated)	return false;
        }
    ...
    for ( unsigned int i = ( unsigned int ) m_toReIntegrateList.size(); i < m_topNActive && i < numFrames; i++ )
    {
        ...
        if (  f->frameIdx>100 && f->dist > m_minPoseDistSqrt && f->type == TrajectoryFrame::Integrated )
        {
...
bool TrajectoryManager::getMaskList ( mat4f& trans, unsigned int frameIdx )
{
    m_mutexUpdateTransforms.lock();
    trans = m_frames[frameIdx].integratedTransform;
    m_mutexUpdateTransforms.unlock();
}

bool TrajectoryManager::getTopFromDeIntegrateList ( mat4f& trans, unsigned int& frameIdx )
{
    if ( m_toDeIntegrateList.empty() )
    {
        return false;
    }
...
{
    if ( m_frames[frameIdx].type == TrajectoryFrame::Invalid )
    {
        return;
    }
