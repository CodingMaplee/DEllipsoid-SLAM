...
extern "C" void extractObjectSurfaceCUDA(const HashDataStruct& hashData,
                                         const RayCastData& rayCastData,
                                         const MarchingCubesParams& params,
                                         MarchingCubesData& data,
                                         uchar objCat);
extern "C" void clearObjectSurfaceCUDA (const HashDataStruct& hashData,
                                         const RayCastData& rayCastData,
                                         const MarchingCubesParams& params,
                                         MarchingCubesData& data,
                                         uchar objCat);

extern "C" void extractDynamicSurfaceCUDA ( const HashDataStruct& hashData,
                                        const RayCastData& rayCastData,
                                        const MarchingCubesParams& params,
                                        MarchingCubesData& data );
...
void CUDAMarchingCubesHashSDF::extractISOSurfaceGPU ( const HashDataStruct& hashData, const HashParams& hashParams, const RayCastData& rayCastData,  const vec3f& minCorner, const vec3f& maxCorner, bool boxEnabled )
{
    //resetMarchingCubesCUDA ( m_data );

    m_params.m_maxCorner = MatrixConversion::toCUDA ( maxCorner );
    m_params.m_minCorner = MatrixConversion::toCUDA ( minCorner );
    m_params.m_boxEnabled = false;
...
MarchingCubesData* CUDAMarchingCubesHashSDF::extractISOSurfaceGPU ( CUDASceneRepChunkGrid& chunkGrid, const RayCastData& rayCastData, const vec3f& camPos, float radius )
{
    // TODO integrate
        ...
        for ( int y = minGridPos.y; y < maxGridPos.y; y++ )
        {
            for ( int z = minGridPos.z; z < maxGridPos.z; z++ )
            {
                ...
                if ( chunkGrid.containsSDFBlocksChunk ( chunk ) )
                {
                    ...
                    //chunkGrid.getHashData(), chunkGrid.getHashParams(), rayCastData, minCorner, maxCorner,
                    extractISOSurfaceGPU ( chunkGrid.getHashData(), chunkGrid.getHashParams(), rayCastData );
...
MarchingCubesData* CUDAMarchingCubesHashSDF::extractIsoSurfaceGPUNoChrunk ( const HashDataStruct& hashData, const HashParams& hashParams, const RayCastData& rayCastData,  const vec3f& minCorner, const vec3f& maxCorner, bool boxEnabled )
{
    resetMarchingCubesCUDA ( m_data );
...
MarchingCubesData* CUDAMarchingCubesHashSDF::extractObjectSurfaceGPUNoChrunk ( const HashDataStruct& hashData, const HashParams& hashParams, const RayCastData& rayCastData, uchar objCat, const vec3f& minCorner, const vec3f& maxCorner, bool boxEnabled)
{
    resetMarchingCubesCUDA ( m_data );
    ...
    extractObjectSurfaceCUDA ( hashData, rayCastData, m_params, m_data, objCat );
    return &m_data;
}

void CUDAMarchingCubesHashSDF::clearObjectSurfaceGPUNoChrunk (const HashDataStruct& hashData, const HashParams& hashParams, const RayCastData& rayCastData, uchar objCat, const vec3f& minCorner, const vec3f& maxCorner, bool boxEnabled)
{
    resetMarchingCubesCUDA ( m_data );
    ...
    clearObjectSurfaceCUDA ( hashData, rayCastData, m_params, m_data, objCat );
}
//MarchingCubesData* CUDAMarchingCubesHashSDF::extractDynamicSurfaceGPUNoChrunk ( const HashDataStruct& hashData, const HashParams& hashParams, const RayCastData& rayCastData, const vec3f& minCorner, const vec3f& maxCorner, bool boxEnabled )
//{
//    resetMarchingCubesCUDA ( m_data );
//
//    m_params.m_maxCorner = MatrixConversion::toCUDA ( maxCorner );
...
//    m_data.updateParams ( m_params );
//
//    extractDynamicSurfaceCUDA ( hashData, rayCastData, m_params, m_data );
//    return &m_data;
//}
